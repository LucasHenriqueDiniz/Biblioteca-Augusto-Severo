...
import { BookGenres } from "../../data/genresArr";
...
const AddButton = ({ updateBooksNow, isAdmin }: AddButtonProps) => {
  ...
  function onSubmit(values: livroType) {
    const livro: livroType = {
      ...
      ano_publicacao: values.ano_publicacao || 0,
      categoria: values.categoria || "Outro",
      capa:
        values.capa ||
    ...
    criarLivro(livro)
      .then((_response) => {
        message.success(livro.titulo + " criado com sucesso!");
        closeModal();
      })
      .catch((error) => {
        if (error.response) {
          message.error(
            `API Request Error: ${error.response.status} (${error.response.statusText})`
          );
        } else {
        ...
        <>
          <Col span={4}>
            <Card
              ...
              title={false}
              hoverable={true}
              className="w-full min-h-[300px] h-full flex justify-center items-center"
            >
          ...
          <Modal
            ...
            destroyOnClose={true}
          >
            <Form layout={"vertical"} onFinish={onSubmit} requiredMark>
              <Form.Item label={"Nome do livro"} name={"titulo"} required>
                <Input />
              </Form.Item>
              <Form.Item label={"Autor"} name={"autor"} required>
                <Input />
              </Form.Item>
              ...
              <Form.Item label={"Genero"} name={"categoria"} required>
                <Select
